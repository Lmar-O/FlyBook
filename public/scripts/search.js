...
clearFiltersButton.addEventListener('click', () => {
    ...
    // Clear the author input field
    authorInputField.value = '';

    // Uncheck all genre radio buttons
    const radio_collection = genresContainer.getElementsByClassName("genre-filter-button");
    for (let i = 0; i < radio_collection.length; i++) {
        radio_collection[i].checked = false;
    }

    // Perform the same action as the Apply Filters button
    const query = searchInput.value;
    let searchStr = `api/search?q=${query}`;

    // No genres or author filters are applied since everything is cleared
    fetch(searchStr)
        .then(response => response.json())
        .then(data => {
            shopItemsContainer.innerHTML = ''; // Clear the shop items container
            ...
            if (data && data.length > 0) {
                ...
                const defaultMarkup = `<div class="center-content"><p class="no-items-feedback">No items available.</p></div>`;
                shopItemsContainer.insertAdjacentHTML('beforeend', defaultMarkup);
            }
        })
        .catch(error => {
